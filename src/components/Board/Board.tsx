...
const Board: React.FC<BoardProps> = ({ currentTurn, onEndTurnCallback, onWinCallback, playerData }) => {
  ...
  const checkDiagonals = (data: number[][]) => {
    if (checkVictory([data[0][0], data[1][1], data[2][2]]) || checkVictory([data[0][2], data[1][1], data[2][0]])) {
      return;
    };
  }
  ...
  const renderTiles = () => {
    return boardData.map((line, lineIndex) => {
      return line.map((each, columnIndex) => {
        return (
          <Pressable
            style={styles.tile}
  ...
  return (
    <View style={[styles.wrapper, winner && styles.wrapper__full]}>
      <View style={[styles.board, winner && styles.board__withFeedback]}>
        {renderTiles()}
      </View>
      {winnerData && (
        <View style={styles.feedback}>
          <View style={{ flexDirection: 'row', alignSelf: 'center', alignItems: 'center' }}>
            <Image
              ...
              source={AVATARS[winnerData.avatarId || '']}
            />
